...
client.on('message', function(topic, message, packet) {
  ...
  switch(topic) {
    ...
    default:
      ...
      var match, zone;
      if( (match = topic.match(/schedule\/(\w+?)\/set/)) != null ) {
        console.dir(match);
        zone = match[1];
        console.log("zone "+zone+" requested to be "+message);
        if( message == 'on' || message == 'off' ) {
          eventbus.emit('override.post', { zone: zone, state: message });
        } else {
          console.log("Ignored, should be on or off");
        }
      } else if( (match = topic.match(/schedule\/(\w+?)\/temperature\/(set|increase|decrease)/)) != null ) {
        zone = match[1];
        var action = match[2];
        console.log("zone "+zone+" requested temperature to be "+message);
        ...
        if( !isNaN( temp ) ) {
          ...
          console.log('Ignoring, not a valid float');
        }
      }
